...
class DataverseMetadataTest < ActiveSupport::TestCase
  ...
  test '#save and .find - saves a Dataverse dataverse_metadata to disk and retrieves it by id' do
    ...
    dataverse_metadata = DataverseMetadata.new
    ...
    retrieved_dataverse_metadata = DataverseMetadata.find(new_id)
    ...
    assert File.exist?(DataverseMetadata.filename_by_id(dataverse_metadata.id))
  ...
  test '#save and .find - saves a Dataverse dataverse_metadata and fails to find another' do
    ...
    retrieved_dataverse_metadata = DataverseMetadata.find(SecureRandom.uuid.to_s)
  ...
  test '.all - returns all saved dataverse_metadatas' do
    dataverse_metadata1 = DataverseMetadata.new.tap do |h|
    ...
    dataverse_metadata2 = DataverseMetadata.new.tap do |h|
      h.id = SecureRandom.uuid.to_s
    ...
    all_dataverse_metadatas = DataverseMetadata.all
  ...
  test '.find_by_uri - finds a dataverse_metadata by its URI' do
    dataverse_metadata = DataverseMetadata.new.tap do |h|
      h.id = SecureRandom.uuid.to_s
    ...
    found_dataverse_metadata = DataverseMetadata.find_by_uri(@sample_uri)
  ...
  test '.find_by_uri - does not find a dataverse_metadata for another URI' do
    ...
    found_dataverse_metadata = DataverseMetadata.find_by_uri(@another_sample_uri)
    assert_nil found_dataverse_metadata
    assert_equal 1, Dir.glob(File.join(DataverseMetadata.metadata_directory, "*.yml")).count
  end

  test '.find_or_initialize_by_uri - initializes a new dataverse_metadata if none found' do
    new_dataverse_metadata = DataverseMetadata.find_or_initialize_by_uri(@sample_uri)
    assert_not_nil new_dataverse_metadata
    assert_equal 'https://example.com:443', new_dataverse_metadata.full_name
    assert File.exist?(DataverseMetadata.filename_by_id(new_dataverse_metadata.id))
    assert_equal 1, Dir.glob(File.join(DataverseMetadata.metadata_directory, "*.yml")).count
  end

  test '.find_or_initialize_by_uri - does not create duplicate dataverse_metadatas' do
    first_dataverse_metadata = DataverseMetadata.find_or_initialize_by_uri(@sample_uri)
    second_dataverse_metadata = DataverseMetadata.find_or_initialize_by_uri(@sample_uri)
    ...
    assert File.exist?(DataverseMetadata.filename_by_id(first_dataverse_metadata.id))
    assert_equal 1, Dir.glob(File.join(DataverseMetadata.metadata_directory, "*.yml")).count
    assert_equal 1, DataverseMetadata.all.count
  end
end
